    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            id="algorithm"
            value={algorithm}
            onChange={(e) => setAlgorithm(e.target.value as 'md5' | 'sha1' | 'sha256')}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            disabled={loading}
          >
      ...
      {result && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <h3 className="text-lg font-semibold text-green-800 mb-3">Результат хеширования</h3>
          
          <div className="space-y-3">
            <div>
              <span className="text-sm font-medium text-gray-600">Алгоритм:</span>
              <span className="ml-2 text-sm text-gray-900">{result.algorithm.toUpperCase()}</span>
            </div>
            
            <div>
              <span className="text-sm font-medium text-gray-600">Исходная строка:</span>
